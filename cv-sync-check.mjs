...
const projectRoot = resolveDataRoot();
...
if (!existsSync(profilePath)) {
  ...
  // Accept either upstream schema (`candidate.full_name`) OR fork's flatter
  // schema (top-level `name:`). Either is fine; we just want to ensure the
  // basic identity fields are present and not still the example-data values.
  const hasIdentityKey = /^(?:name|full_name)\s*:/m.test(profileContent)
    || /^candidate\s*:[\s\S]*?(?:name|full_name)\s*:/m.test(profileContent);
  const hasEmail = /^email\s*:/m.test(profileContent)
    || /^\s+email\s*:/m.test(profileContent);
  const hasLocation = /^location\s*:/m.test(profileContent)
    || /^\s+location\s*:/m.test(profileContent);
  const stillExample = profileContent.includes('"Jane Smith"')
    || profileContent.includes('jane@example.com');
  if (!hasIdentityKey) warnings.push('config/profile.yml: missing name/full_name field');
  if (!hasEmail) warnings.push('config/profile.yml: missing email field');
  if (!hasLocation) warnings.push('config/profile.yml: missing location field');
  if (stillExample) warnings.push('config/profile.yml: still contains example data (Jane Smith / jane@example.com)');
}
