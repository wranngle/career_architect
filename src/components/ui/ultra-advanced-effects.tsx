      ...
      for (let i = 0; i < 5; i++) {
        ...
        for (let j = 0; j <= points; j++) {
          ...
          if (j === 0) {
            ...
            const previousAngle = ((j - 1) / points) * Math.PI * 2;
            const previousR = radius + Math.sin(time * 3 + (j - 1) + i) * 20;
            const previousX = x + Math.cos(previousAngle) * previousR;
            const previousY = y + Math.sin(previousAngle) * previousR;

            const cpx = (previousX + px) / 2 + Math.sin(time + j) * 10;
            const cpy = (previousY + py) / 2 + Math.cos(time + j) * 10;
