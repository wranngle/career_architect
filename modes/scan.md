...
## Search Tier Ladder (extended — agent flow)

The 3-level model above describes what `scan.mjs` does. When an agent is
running an exhaustive search (not just `scan.mjs`), follow this 5-tier
ladder for maximum coverage. **Higher tiers MANDATORY before lower
tiers — start at Tier 1 and proceed sequentially.**
...
| 5 | Playwright last-resort | Anything Tier 2-4 missed due to bot protection / JS-heavy SPA / login wall | Conditional — must be explicitly justified in the coverage checklist below |

## Exa query patterns library

> **Canonical Exa reference:** https://docs.exa.ai/reference/search-api-guide-for-coding-agents
> If anything below contradicts real API behavior, that URL is the source of truth.
> The `exa` MCP server in `.mcp.json` exposes Exa via HTTP transport; the
> `EXA_API_KEY` in `.env` is for direct `curl`/Python calls when an agent
> wants raw JSON instead of going through MCP.

Reusable templates for Tier 2-4 Exa searches. `{kw}` = role keywords,
`{tf}` = recency window (e.g. `last 7 days`, `posted 2026`). Recommended
search type per tier: Tier 2 site-targeted = `fast`/`auto`; Tier 3 broad
= `auto`; Tier 4 academic = `deep` for synthesis. Pair with
`outputSchema` when extracting structured fields (company name, salary
band) instead of parsing free text.

- Site-targeted:        `site:remoteok.io {kw}`
- Multi-site:           `site:wellfound.com OR site:angel.co {kw} remote`
- Temporal:             `{kw} site:linkedin.com/jobs posted {tf}`
- Salary-anchored:      `{kw} salary $120k-$180k remote`
- Accessibility:        `{kw} autism-friendly OR neurodivergent-friendly OR sensory-accommodation`
- Visa-friendly:        `{kw} "visa sponsorship" OR "H-1B sponsor" OR "relocation package"`
- Equity-disclosing:    `{kw} "equity" "salary" "remote" -recruiter`

Compose 2-3 patterns per role search. Skip patterns that don't match the
role's intent (e.g. don't run "accessibility" pattern for a Defense
Department clearance role).

## SEARCH COVERAGE CHECKLIST

**MANDATORY** at the end of every exhaustive scan. Append this block to
the scan output report and mark every line with one of: `[X jobs] / [Error: <msg>] / [No results] / [Skipped: <reason>]`.

```
SEARCH COVERAGE CHECKLIST — {date}
- Tier 1a (scan.mjs ATS APIs):
  - [ ] Greenhouse:   [...]
  - [ ] Ashby:        [...]
  - [ ] Lever:        [...]
- Tier 1b (scan-jobspy.py):
  - [ ] Indeed:       [...]
  - [ ] LinkedIn:     [...]
  - [ ] Glassdoor:    [...]
  - [ ] ZipRecruiter: [...]
...
- Tier 3 (Exa broad):
  ...
  - [ ] Regional:     [...]
- Tier 4 (Exa academic):
  - [ ] Academic:     [Skipped: non-academic role]
- Issues:
  - Rate limits hit:  [list or "None"]
  - Errors:           [describe or "None"]
  - Playwright (T5):  [Justify per-site or "No"]
```

Skipping a tier without a checklist entry is a coverage bug — the user
trusts the system to be exhaustive.

## JobSpy optimization tactics (for `scan-jobspy.py` agent flow)

When invoking JobSpy from an agent (not `scan.mjs`), follow these
patterns to avoid rate-limits and dead results:

- **Platform rotation**: spread the same query across `indeed`, `linkedin`,
  `glassdoor` rather than asking one platform for 100+ results.
- **Keyword variation**: run 2-3 queries with synonym variants
  ("Senior Backend Engineer" / "Senior Software Engineer Backend" /
  "Senior Platform Engineer") to widen the recall net.
- **Date filtering**: pass `hours_old=168` (7 days) for routine scans,
  `hours_old=24` for daily refreshes. Avoid `hours_old=0` (no filter)
  unless explicitly bootstrapping.
- **Easy-apply filtering**: avoid `easy_apply=True` for senior roles —
  it skews toward low-effort listings. Default `easy_apply=None`.
- **Country selection**: set `country_indeed` explicitly per scan;
  defaults often pull stale geo.
- **Concurrency**: max 1 JobSpy invocation in flight per platform per
  minute; the underlying scrapers will 429 hard if hammered.

## Memory MCP usage (cross-session continuity)

If the `memory` MCP server is configured (it is — see `.mcp.json`),
record per-scan and per-application state so future sessions don't
re-do work:

- After each scan, store an entity `Scan:{date}` with relations
  `discovered → {company}::{role}` for every new posting kept.
- After each evaluation, store `Application:{company}::{role}` with
  observations `[score, status, last_action_date, contact_name]`.
- Before re-scanning a company, query the graph for any
  `Application:{company}::*` to surface in-flight applications and
  avoid duplicate outreach.
- Before drafting outreach, query for prior `Contact:{name}` notes.

This keeps the agent stateful across ephemeral cloud-sandbox sessions
