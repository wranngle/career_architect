...
## H) 10-Dimension Weighted Score
(JSON block — see below for structure. THIS is the source of the **Score:** in the header.)
...
### Block H format (mandatory in every report)

A fenced ```json block with EXACTLY these keys. Each dimension scored 1-5
(integers; halves OK). The final `score` is the weighted sum. Default
weights below; override per-candidate in `modes/_profile.md` or
`config/profile.yml` under `scoring_weights:`.

```json
{
  "dimensions": {
    "role_match":         { "score": 0, "weight": 0.20, "note": "" },
    "seniority_fit":      { "score": 0, "weight": 0.10, "note": "" },
    "tech_stack_overlap": { "score": 0, "weight": 0.15, "note": "" },
    "comp":               { "score": 0, "weight": 0.10, "note": "" },
    "remote_policy":      { "score": 0, "weight": 0.10, "note": "" },
    "company_stage":      { "score": 0, "weight": 0.05, "note": "" },
    "mission_fit":        { "score": 0, "weight": 0.10, "note": "" },
    "growth_trajectory":  { "score": 0, "weight": 0.05, "note": "" },
    "recruiter_quality":  { "score": 0, "weight": 0.05, "note": "" },
    "red_flags":          { "score": 0, "weight": 0.10, "note": "5=none, 1=many" }
  },
  "score": 0.0,
  "score_band": "high|mid|low|skip"
}
```

`score` = weighted sum, rounded to 1 decimal.
`score_band`: high ≥ 4.5, mid 4.0-4.49, low 3.0-3.99, skip < 3.0.

The `score` from this block IS the final score. Use it in the header,
the tracker line, and the auto-pipeline gate (4.0 hard-stop).
